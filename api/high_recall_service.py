...
class HighRecallService:
    ...
    def __get_document_vector_sum(self):
        ...
        for doc in self.corpus_tokenized:
            word_vectors = [self.vectors[word] for word in doc if word in self.vectors.vocab]
    ...
    def get_N_similar_questions(self, question):
        ...
        #konvertujem input u string
        question_str = str(question)

        #tokenizujem ulazno pitanje
        question_tokenized = nltk.word_tokenize(question_str.lower())

        #vektor ulaznog pitanja
        question_vector = np.sum([self.vectors[word] for word in question_tokenized if word in self.vectors.vocab], axis=0)

        #nalazim 100 najslicnijih pitanja i vracam
